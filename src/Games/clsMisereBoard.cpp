...
clsMisereBoard::clsMisereBoard() : clsBoard(3, 3) {
  for (auto &row : vBoard)
    for (auto &cell : row)
      cell = _blankSymbol;
}

bool clsMisereBoard::updateBoard(clsMove<char> *move) {
  int x = move->getX();
  int y = move->getY();
...
bool clsMisereBoard::isWin(clsPlayer<char> *player) {
...
bool clsMisereBoard::isDraw(clsPlayer<char> *player) {
  return (nMoves == 9 && !isWin(player));
}

bool clsMisereBoard::isLose(clsPlayer<char> *player) {
  char symbol = player->getSymbol();
...
bool clsMisereBoard::gameIsOver(clsPlayer<char> *player) {
