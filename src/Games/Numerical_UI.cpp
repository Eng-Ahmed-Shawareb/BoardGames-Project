...
void Numerical_UI::print_avaliable_numbers(int num_type) {
  if (num_type % 2) {
    cout << "[ ";
    for (auto it = avaliable_odd_numbers.begin();
         it != avaliable_odd_numbers.end(); ++it) {
      cout << *it << " ";
    }
    cout << "]";
    ...
    cout << "[ ";
    for (auto it = avaliable_even_numbers.begin();
         it != avaliable_even_numbers.end(); ++it) {
      cout << *it << " ";
    }
    cout << "]";
  }
...
Move<int> *Numerical_UI::get_move(Player<int> *player) {
  ...
  if (player->get_type() == PlayerType::HUMAN) {
    ...
    if (num_type % 2) {
      ...
      while (avaliable_odd_numbers.find(symbol) ==
             avaliable_odd_numbers.end()) {
        cout << "Invalid number. ";
        cout << "Please enter number from : ";
        print_avaliable_numbers(num_type);
        cout << endl;
      ...
      while (avaliable_even_numbers.find(symbol) ==
             avaliable_even_numbers.end()) {
        cout << "Invalid number. ";
        cout << "Please enter number from : ";
        print_avaliable_numbers(num_type);
        cout << endl;
...
int Numerical_UI::min_max(Board<int> *current_board, int depth, int alpha,
                          int beta, bool is_max, int AI_type, int human_type) {
  // base case
      ...
      for (int j = 0; j < current_board->get_columns(); ++j) {
        if (current_board->get_cell(i, j) == 0) {
          ...
          for (int num : choices) {
            ...
            (AI_type % 2) ? avaliable_odd_numbers.erase(num)
                          : avaliable_even_numbers.erase(num);
            ...
            (AI_type % 2) ? avaliable_odd_numbers.insert(num)
                          : avaliable_even_numbers.insert(num);
      ...
      for (int j = 0; j < current_board->get_columns(); ++j) {
        if (current_board->get_cell(i, j) == 0) {
          ...
          for (int num : choices) {
            ...
            (human_type % 2) ? avaliable_odd_numbers.erase(num)
                             : avaliable_even_numbers.erase(num);
            ...
            Move<int> undo_move(i, j, 0);
            current_board->update_board(&undo_move);
            (human_type % 2) ? avaliable_odd_numbers.insert(num)
                             : avaliable_even_numbers.insert(num);
...
Move<int> *Numerical_UI::get_AI_move(Player<int> *AI_player) {
  ...
  *test_board = *AI_player->get_board_ptr();
    ...
    for (int j = 0; j < test_board->get_columns(); ++j) {
      if (test_board->get_cell(i, j) == 0) {
        ...
        for (int num : choices) {
          ...
          (AI_player->get_symbol() % 2) ? avaliable_odd_numbers.erase(num)
                                        : avaliable_even_numbers.erase(num);

          int score = min_max(test_board, 0, best_score, 1e5, false, num_type,
                              1 - num_type);

          Move<int> undo_move(i, j, 0);
          test_board->update_board(&undo_move);

          (AI_player->get_symbol() % 2) ? avaliable_odd_numbers.insert(num)
                                        : avaliable_even_numbers.insert(num);
  ...
  (AI_player->get_symbol() % 2)
      ? avaliable_odd_numbers.erase(best_move->get_symbol())
