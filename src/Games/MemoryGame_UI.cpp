...
int MemoryGame_UI::min_max(Board<char> *current_board, int depth, bool is_max,
                           char AI_symbol, char human_symbol) {
    ...
    for (int i = 0; i < current_board->get_rows(); ++i)
      for (int j = 0; j < current_board->get_columns(); ++j) {
        if (current_board->get_cell(i, j) == '.') {
          ...
          int score =
              min_max(current_board, depth + 1, false, AI_symbol, human_symbol);
    ...
    for (int i = 0; i < current_board->get_rows(); ++i)
      for (int j = 0; j < current_board->get_columns(); ++j) {
        ...
        if (current_board->get_cell(i, j) == '.') {
          ...
          current_board->update_board(&current_move);
          int score =
              min_max(current_board, depth + 1, true, AI_symbol, human_symbol);
          Move<char> undo_move(i, j, 0);
          current_board->update_board(&undo_move);
  ...
  for (int i = 0; i < test_board->get_rows(); ++i)
    for (int j = 0; j < test_board->get_columns(); ++j) {
      ...
      if (test_board->get_cell(i, j) == '.') {
        ...
        int score = min_max(test_board, 0, false, AI_player->get_symbol(),
                            (AI_player->get_symbol() == 'X') ? 'O' : 'X');
