...
int SUS_UI::min_max(SUS_Board *current_board, bool is_max, char AI_symbol,
                    char human_symbol) {
  // base case
  ...
  for (int i = 0; i < test_board->get_rows(); ++i)
    for (int j = 0; j < test_board->get_columns(); ++j) {
      ...
      if (test_board->get_cell(i, j) == '.') {
        ...
        int score = min_max(test_board, false, player->get_symbol(),
                            (player->get_symbol() == 'S') ? 'U' : 'S');
...
Player<char> *SUS_UI::create_player(string &name, char symbol,
                                    PlayerType type) {
